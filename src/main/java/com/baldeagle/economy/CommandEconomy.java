        ...
        try {
            if (type.equalsIgnoreCase("player")) {
                ...
                if (action.equalsIgnoreCase("get")) {
                    ...
                    sender.sendMessage(new TextComponentString(targetPlayer.getName() + " balance: " + balance));
                } else if (args.length >= 4) {
                    long amount = Long.parseLong(args[3]);
                    if (action.equalsIgnoreCase("deposit")) {
                        EconomyManager.depositPlayer(world, uuid, amount);
                        sender.sendMessage(new TextComponentString("Deposited " + amount + " to " + targetPlayer.getName()));
                    } else if (action.equalsIgnoreCase("withdraw")) {
                        if (EconomyManager.withdrawPlayer(world, uuid, amount)) {
                            sender.sendMessage(new TextComponentString("Withdrew " + amount + " from " + targetPlayer.getName()));
                        } else {
                ...
                if (action.equalsIgnoreCase("get")) {
                    ...
                    sender.sendMessage(new TextComponentString(country + " balance: " + balance));
                } else if (args.length >= 4) {
                    long amount = Long.parseLong(args[3]);
                    if (action.equalsIgnoreCase("deposit")) {
                        EconomyManager.depositCountry(world, country, amount);
                        sender.sendMessage(new TextComponentString("Deposited " + amount + " to " + country));
                    } else if (action.equalsIgnoreCase("withdraw")) {
                        if (EconomyManager.withdrawCountry(world, country, amount)) {
                            sender.sendMessage(new TextComponentString("Withdrew " + amount + " from " + country));
                        } else {
                            sender.sendMessage(new TextComponentString("Insufficient funds!"));
                        }
                    }
